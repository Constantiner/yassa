...
Two workflows can run depending on event type:

1. `CI` workflow (`ci.yml`)
2. `Release` workflow (`release.yml`) with two jobs:
    - `version-pr` job (`push` on `main`)
    - `publish` job (`pull_request_target` when `changeset-release/main` is merged into `main`)

Why this shape exists:

- Changesets release commits include `[skip ci]`.
- GitHub skips `push`/`pull_request` workflows for commits with `[skip ci]`.
- GitHub does **not** skip `pull_request_target` workflows for that case.
- So publish must be triggered from merged PR event (`pull_request_target`), not from post-merge push commit.

Behavior:

- After a feature PR with changesets is merged to `main`, `version-pr` opens/updates the "Version Packages" PR.
- After "Version Packages" is merged, `publish` job runs (guarded to that specific head branch).
- Publish uses `changesets/action@v1` + `npm run release:publish`.
- On success, npm package is published and GitHub tag/release are created automatically.
...
After release PR merge, `publish` job is triggered by `pull_request_target` and executes:
...
This repository now avoids that failure mode by publishing on `pull_request_target` for merged `changeset-release/main`.
...
- `.github/workflows/release.yml`:
    - trigger for version PR: `on.push.branches = [main]`
    - trigger for publish: `on.pull_request_target.types = [closed]` on `main`
    - manual recovery trigger: `on.workflow_dispatch`
    - `publish` job guard requires merged PR head branch `changeset-release/main`
- publish job permissions include:
    - `contents: write`
    - `id-token: write`
...
- release PR merged but publish guard did not match (wrong head branch / event mismatch).
- release workflow failed in the `publish` job.

Fix:

1. Merge release PR.
2. Verify merged PR head ref is exactly `changeset-release/main`.
3. Confirm a `Release` workflow run exists for `pull_request_target` closed/merged event.
4. If publish failed, inspect `Publish via Changesets` step and re-run workflow.
